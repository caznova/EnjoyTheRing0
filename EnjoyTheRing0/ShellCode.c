...
BOOL __fastcall PrepareUsermodeMemory(PUM_MEMORY_INFO UmMemoryInfo, PMEMORY_DESCRIPTOR MemoryDescriptor) {
...
VOID __fastcall FreePreparedMemory(PMEMORY_DESCRIPTOR MemoryDescriptor) {
...
BOOL __fastcall PrepareIoBuffer(
	...
	if (!IsUsermodeMemoryReadable(MemoryInfo, sizeof(UM_MEMORY_INFO), 1)) return FALSE;
	...
	switch (ShellBufferType) {
		case ShellInputBuffer:
			if (!IsUsermodeMemoryReadable(MemoryInfo->Address, MemoryInfo->Size, 1)) return FALSE;
			break;

		case ShellOutputBuffer:
			if (!IsUsermodeMemoryWriteable(MemoryInfo->Address, MemoryInfo->Size, 1)) return FALSE;
			break;
	}
...
SHELL_STATUS ExecuteShell(
	...
	__try {
		...
		if (Result) {
			if (IsUsermodeMemoryWriteable(Result, sizeof(SIZE_T), 1)) {
				*Result = ShellResult;
			} else {
