...
VOID __inline FreeNtPath(LPWSTR NtPath) {
	FreeString(NtPath);
}

SIZE_T __inline InitUnicodeNtPath(LPWSTR Path, OUT PUNICODE_STRING NativePath, OUT LPWSTR* NtPath) {
	SIZE_T Length = GetNtPath(Path, NtPath);
	RtlInitUnicodeString(NativePath, *NtPath);
	return Length;
}

VOID __inline FreeUnicodeNtPath(LPWSTR NtPath) {
	FreeString(NtPath);
}
...
NTSTATUS CloseFile(HANDLE hFile) {
